...
my %gCfg = graphHoursConfig(
	"Voltage Log: hourly averages",		# Graph Title
	\@hlog,				# hash reference to the data log
	"FX_BATT_V",			# key to match in logs
	\%time,				# time hash
	"V",['AVG','MAX','MIN'],	# Voltage and graph legend prefix
	$print,				# Graph color scheme (print or not)
	40,65,				# Default min and max y-axis values
	\%cfg);
